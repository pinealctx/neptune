...
func test100KLRU(t *testing.T, size int64, rc int, wc int) {
	...
	lru = NewWideXHashLRU(size)
	test100KPassLRU(t, "73", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(7))
	test100KPassLRU(t, "7", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(13))
	test100KPassLRU(t, "13", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(31))
	test100KPassLRU(t, "31", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(211))
	test100KPassLRU(t, "211", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(251))
	test100KPassLRU(t, "251", lru, rc, wc)

	lru = NewWideXHashLRU(size, remap.WithPrime(509))
