...
func (b Bit64) IterAsI64(s []int64, pos int, add int64, n int) int {
	...
	if l > int(dirIter) {
		...
		var i int
		for w != 0 {
			i = bits.TrailingZeros64(uint64(w))
			if c >= n || c >= l {
				break
			}
			s[cursor] = int64(i) + add
			cursor++
			c++
			w &= ^u64Tab[i]
		}
